...
from innexiaBot import TEMP_DOWNLOAD_DIRECTORY
from innexiaBot import telethn as client
...
async def _(event):
    ...
    if event.is_group:
        if not (await is_register_admin(event.input_chat, event.message.sender_id)):
            await event.reply(
                "Hey, You are not admin. You can't use this command, But you can use in my pm ^_^"
            )
            return
